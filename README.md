...
Стиль кода должен соответствовать [PSR-1](http://www.php-fig.org/psr/psr-1/), [PSR-2](http://www.php-fig.org/psr/psr-2/) и [PSR-12](https://www.php-fig.org/psr/psr-12/).
...
Если метод ничего не возвращает, это должно быть указано как `void`.
Это проясняет для пользователей вашего кода, что вы намеревались написать.
...
Не используйте докблоки для методов, которые могут иметь полную подсказку типа (если вам не нужно описание).

Добавляйте описание только в том случае, если оно предоставляет больше контекста, чем сама подпись метода. Используйте полные предложения для описаний, включая точку в конце.
...
Всегда используйте полностью определенные имена классов в блоках документации.
...
По возможности, докблоки следует записывать в одну строку.
...
Если переменная имеет несколько типов, первым должен быть наиболее часто встречающийся тип.
